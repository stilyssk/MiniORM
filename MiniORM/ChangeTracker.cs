...
namespace MiniORM
{
	...
	{
		private readonly List<T> allEntityes;
		private readonly List<T> added;
		private readonly List<T> removed;

		public IReadOnlyCollection<T> AllEntities => this.allEntityes.AsReadOnly();
		...
		public ChangeTracker(IEnumerable<T> entities)
		{
			...
			this.allEntityes = CloneEntities(entities);
		}
		...
		public IEnumerable<T> GetModifiedEntities (DbSet<T> dbSet)
		{
			...
			foreach (var proxyEntity in this.AllEntities)
			{
				...
				var isModfied = IsModified(proxyEntity, entity);
				if (isModfied)
				{
